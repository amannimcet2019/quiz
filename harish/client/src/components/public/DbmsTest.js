...
let DbmsTest=(props)=>{
    ...
    useEffect(()=>fun(),[])    
    const fun=async()=>{        
        const response = await Axios.get(`http://localhost:5000/dbms/${props.match.params.id}`);
        setTest({...response.data.dbms})
        if(response.data.dbms)
            setFlag(1)
        else 
            setFlag(0)
    }
    if(flag==0)
        ...
        return(<React.Fragment>
            <h2 className="text-center mt-3 text-info">Compilation DBMS Test {test.testno}</h2>
    ...
    return(
        <React.Fragment>
            ...
            <div className="container">
                <div className="row">
                    <div className="col-md-9 mx-auto">
                    <h2 className="text-center mt-3 text-danger">DBMS Test {test.testno}</h2>
                        {/* question 1 */}
